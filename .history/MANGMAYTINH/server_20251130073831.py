    ...
    while not state.is_stopped():
        ...
        if type_ == TYPE_TEXT:
            ...
            if msg == "play":
                ...
                send_packet(sock, TYPE_TEXT, b"SERVER: UNKNOWN COMMAND (use play/pause/quit)")


def stream_video(sock: socket.socket, state: ServerState, video_path=None):
    """Gửi frame liên tục khi ở trạng thái PLAYING."""
    vs = VideoStream(video_path=video_path)  # None -> webcam
    try:
        while not state.is_stopped():
            if state.is_playing():
                ...
                # đang pause -> ngủ tí cho đỡ tốn CPU
                time.sleep(0.05)
    finally:
        vs.release()
        sock.close()
...
def main():
    ...
    control_t = threading.Thread(target=handle_control, args=(client_sock, state), daemon=True)
    control_t.start()

    # truyền từ file video: truyền đường dẫn vào video_path,
    # hoặc để None để dùng webcam
    stream_video(client_sock, state, video_path="movie.mp4")
