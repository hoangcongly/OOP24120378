...
class VideoStream:
    def __init__(self, video_path: Optional[str] = None, cam_index: int = 0):
        """
        Nếu có video_path -> đọc từ file.
        Không có -> đọc từ webcam (cam_index).
        """
    ...
    def get_next_jpeg(self, quality: int = 80) -> Optional[bytes]:
        """Đọc frame tiếp theo và trả về bytes JPEG. Hết video -> None."""
...
def jpeg_to_frame(data: bytes):
    """Phía client: bytes JPEG -> frame OpenCV để hiển thị."""
