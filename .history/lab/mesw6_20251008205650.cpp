...
void CListMessage::Top10()
{
    ...
    vector<pair<int, int>> sortedVotes(voteCount.begin(), voteCount.end());
    sort(sortedVotes.begin(), sortedVotes.end(),
         [](const pair<int, int>& a, const pair<int, int>& b)
         {
             if (a.second != b.second)
                 return a.second > b.second; // sắp giảm dần theo lượt bình chọn
             return a.first < b.first;       // nếu bằng nhau thì theo mã tăng dần
         });
    ...
    for (const auto& pair : sortedVotes)
    {
        if (rank > 10)
            break;
        cout << "Hang " << rank << ": Ma so " << pair.first
             << " - " << pair.second << " luot binh chon.\n";
        rank++;
    }
}