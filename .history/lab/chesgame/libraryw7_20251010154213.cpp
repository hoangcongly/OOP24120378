...
void CLibrary::SearchByAuthor()
{
    ...
    if(!found)
    {
        std:: cout << "Khong tim thay tai lieu nao cua tac gia co ten chua: " << author_name << std:: endl;
    }
}
void CLibrary:: FilterByType()
{
    std:: string type_str;
    std::cout << "Nhap loai can loc (book, paper, thesis): ";
    std::cin >> type_str;
    std:: transform(type_str.begin(), type_str.end(),type_str.begin(), ::tolower);
    std:: string type_code;
    if(type_str=="book") type_code="B";
    else if(type_str=="paper") type_code="P";
    else if(type_str=="thesis") type_code="T";
    else
    {
        std:: cout << "Loai khong hop le\n";
        return;
    }
    std:: cout << "Cac tai lieu thuoc loai " << type_str << " la: " << std:: endl;
    bool found=false;
    for(const auto&cat: m_Catalogues)
    {
        if(cat->getType()==type_code)
        {
            cat->Output(std:: cout);
            std::cout << std:: endl;
            found=true;
    ...
    if(!found)
    {
        std::cout << "Khong co tai lieu nao thuoc loai nay\n";
    }
}