    ...
    while (window.isOpen()) {
        ...
        while (optEvent.has_value()) {
            ...
            if (event.is<sf::Event::MouseButtonPressed>()) {
                const auto& mouseEvent = event.get<sf::Event::MouseButtonEvent>();

                if (mouseEvent.button == sf::Mouse::Button::Left) {
                    if ((0 <= mouseEvent.x) && (mouseEvent.x <= 512) &&
                        (0 <= mouseEvent.y) && (mouseEvent.y <= 512)) {
                    ...
                    } else if ((517 <= mouseEvent.x) && (mouseEvent.x <= 763) &&
                               (5 <= mouseEvent.y) && (mouseEvent.y <= 45)) {
                        chess.restart();
                    }
                }
            }
