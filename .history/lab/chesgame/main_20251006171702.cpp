...
int main() {
    ...
    sf::RenderWindow window(sf::VideoMode(768u, 512u), "Chess", sf::Style::Titlebar | sf::Style::Close);
        ...
        while (window.pollEvent(event)) {
            ...
            if (event.type == sf::Event::MouseButtonPressed) {
                if (event.mouseButton.button == sf::Mouse::Left) {
                    ...
                    } else if ((517 <= event.mouseButton.x) && (event.mouseButton.x <= 763) &&
                               (5 <= event.mouseButton.y) && (event.mouseButton.y <= 45)) {
                        chess.restart();
                    }
                }
            }
