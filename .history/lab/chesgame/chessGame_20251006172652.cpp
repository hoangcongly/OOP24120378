...
: board(bordCol1, bordCol2), selected(false), playerTurn(true), mate(false), turn(1)
{
    ...
    std::cerr << "Error: cannot load font 'Textures/arial.ttf'\n";
...
void chessGame:: calcQueenMoves(Piece*tmpPiece)
{
    ...
    while((piecePos-posCounter)>=0&& ((piecePos-posCounter)%8<(piecePos-8)))
    {
        ...
        if(!dangerMove)
        {
            ...
            if(!playerTurn)
            {
                if((piecePos-posCounter)==whitePieces[3].getPosition())
                {
                    dangerMove=true;
                }
            }
...
void chessGame:: calcRookMoves(Piece*tmpPiece)
{
    ...
    if(!dangerMove) tmpPiece->getPossibleMoves().clear();
    if(!tmpPiece->getDangerMoves().empty())
    {
        int collisions=0;
        for(int j=0; j<tmpPiece->getDangerMoves().size(); j++)
