...
template <class T>
void SLList<T>::remove(Iterator pos)
{
    Node *target = pos.current_node;
    if (target == nullptr || _pHead == nullptr)
    {
        return;
    }
    if (target == _pHead)
    ...
    Node *prev = _pHead;
    while (prev != nullptr && prev->_pNext != target)
    {
        prev = prev->_pNext;
    }
    if (prev == nullptr)
